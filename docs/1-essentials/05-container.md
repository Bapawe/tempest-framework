...
The constructors of classes resolved by the container may be any class or interface associated with a [dependency initializer](#dependency-initializers). Similarly, invoked methods such as [event handlers](../2-features/08-events.md), [console commands](../1-essentials/04-console-commands.md) and invokable classes may also be called directly from the container.
...
This is specifically useful to get multiple instances of the same configuration. This is how [multiple database connections support](../1-essentials/03-database.md#connecting-to-multiple-databases) is implemented.
